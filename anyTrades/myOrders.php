    ...
    <?php require "alert.php"; ?>
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                $pagination_rs = Database::search("SELECT * FROM `invoice` WHERE `user_email`='" . $user["email"] . "' AND `status`<>2 AND order_status_id<>5 ORDER BY date_time ASC");
                ...
                $rs  = Database::search("SELECT * FROM `invoice` WHERE `user_email`='" . $user["email"] . "' AND `status`<>2 AND order_status_id<>5 ORDER BY date_time ASC LIMIT " . $result_per_page . " OFFSET " . $offset . "");
                $num = $rs->num_rows;
